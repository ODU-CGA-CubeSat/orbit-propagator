...
system('build/VintiCode')

%Get Data from Output of Vinti program
VintiOutput = csvread("build/outputStateVect.txt");
...
altitude_loop(i) = round((norm([x_ECI(1,i) x_ECI(2,i) x_ECI(3,i)]) - r_earth)*100)/100;           %km
Veloc_loop(i,:) = [x_ECI(4,i) x_ECI(5,i) x_ECI(6,i)]*1000; V(i) = norm(Veloc_loop(i,:));    %m/s
velocUnitVector(i,:) = Veloc_loop(i,:)./V(i);
FD_avg(i) = DragParam(round((altitude_loop(i)-0)/.01+1),2) * V(i)^2;                         %N
...
csvwrite("build/inputStateVect.txt",x_ECI(:,i))
end
